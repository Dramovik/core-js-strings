 ...
 *   removeLastOccurrences('I like legends', 'end') => 'I like legs'.13
 *   removeLastOccurrences('ABABAB', 'BA') => 'ABAB'.
 */
function removeLastOccurrences(str, value) {
  const numSearch = str.lastIndexOf(value);
  if (numSearch >= 0) {
    const subStr1 = str.slice(0, numSearch);
    const subStr2 = str.slice(numSearch + value.length, str.length);
...
/**
 * возвращаетСтрокуВОбратномПорядке
 *
 * @param {string} str - The input string.
...
function isPalindrome(str) {
  ...
  const checkPalindrome = copyStr.split('').reverse().join('');
  if (copyStr === checkPalindrome) {
    return true;
  }
  return false;
}
...
// Perhaps to navigate in large strings you don't need to use sorting, but rather iterate over each element
...
function unbracketTag(str) {
  const tempStr = str.replace('<', '').replace('>', '');
  return tempStr;
}
