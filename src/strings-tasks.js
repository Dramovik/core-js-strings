...
function removeFirstOccurrences(str, value) {
  const indexSearchStr = str.indexOf(value);
  if (indexSearchStr >= 0) {
    return `${str.slice(0, indexSearchStr)}${str.slice(indexSearchStr + value.length, str.length)}`;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indexSearchStr = str.lastIndexOf(value);
  if (indexSearchStr >= 0) {
    return `${str.slice(0, indexSearchStr)}${str.slice(indexSearchStr + value.length, str.length)}`;
  }
  return str;
...
function isPalindrome(str) {
  const preparedStr = str
    ...
    .replaceAll('?', '')
    .replaceAll(' ', '');
  return preparedStr.split('').reverse().join('') === preparedStr;
}
...
function encodeToRot13(str) {
  ...
  const DECRYPT = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = DECRYPT.indexOf(str[i]);
    if (index >= 0) {
      result += ENCRYPT[index];
    } else {
      result += str[i];
    }
  }
...
function getCardId(value) {
  ...
  return CARD_LIST.indexOf(value);
}
